...
#define RD_buff _IOR('a','0',arg_t*)
#define WR_buff _IOW('a','1',arg_t*)
...
static int my_open(struct inode *inode, struct file *file)
{
        printk(KERN_ALERT"Device File Opened...!!!\n");
        return 0;
}

static int my_release(struct inode *inode, struct file *file)
{
        printk(KERN_ALERT"Device File Closed...!!!\n");
        return 0;
}
...
static long my_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
         switch(cmd) {
                ...
                case WR_buff:
                        if( copy_from_user(&arg_t ,(char*) arg, sizeof(arg_t)) )
                        {
                        ...
                        break;
                case RD_buff:
                        if( copy_to_user((char*) arg, &arg_t, sizeof(arg_t)) )
                        {
...
static int __init my_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "IO_DEVICE")) <0){
        ...
        printk(KERN_ALERT"Device Driver Insert...Done!!!\n");
        return 0;
